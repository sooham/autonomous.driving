...
train = data['train'][486:]
valid = data['train'][:486]
test = data['val']
hidden = data['test_128']

l_train = data['train_y_single'][486:] - 1
l_valid = data['train_y_single'][:486] - 1
...
labels_train[np.arange(n_data), l_train.astype(np.uint8)] = 1.0

labels_valid = np.zeros((n_valid, n_classes), dtype=np.float32)
labels_valid[np.arange(n_valid), l_valid.astype(np.uint8)] = 1.0
# computations
...
dropout = 1.0
momentum = 0.01          # used if momentum implemented
...
weights = {
    ...
    'wd1': tf.Variable(tf.truncated_normal([8*8*256, 3069], stddev=0.1)),
    # 1024 inputs, 10 outputs (class prediction)
    'out': tf.Variable(tf.truncated_normal([3069, n_classes], stddev=0.1))
}

biases = {
    'bc1': tf.Variable(tf.constant(0.00001, shape=[56])),
    'bc2': tf.Variable(tf.constant(0.00001, shape=[56])),
    'bc3': tf.Variable(tf.constant(0.00001, shape=[64])),
    'bc4': tf.Variable(tf.constant(0.00001, shape=[112])),
    'bc5': tf.Variable(tf.constant(0.00001, shape=[256])),
    'bd1': tf.Variable(tf.constant(0.00001, shape=[3069])),
    'out_b': tf.Variable(tf.constant(0.00001, shape=[n_classes]))
}

# Construct model
logits = conv_net(x, weights, biases, keep_prob)
logits_valid = conv_net(tf.constant(valid, tf.float32), weights, biases, tf.constant(1., tf.float32))

# Define loss and optimizer
regularization = tf.nn.l2_loss(weights['out']) + tf.nn.l2_loss(weights['wd1'])
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits, y)) + l2_loss_size * regularization

optimizer = tf.train.MomentumOptimizer(learning_rate, momentum).minimize(cost)

# Evaluate model
predictions = tf.argmax(logits, 1)
predictions_valid = tf.argmax(logits_valid, 1)

correct_pred = tf.cast(tf.equal(predictions, tf.argmax(y, 1)), tf.float32)
correct_pred_valid = tf.cast(tf.equal(predictions_valid, tf.argmax(tf.constant(labels_valid, tf.float32), 1)), tf.float32)

accuracy = tf.reduce_mean(correct_pred)
accuracy_valid = tf.reduce_mean(correct_pred_valid)
...
tf.scalar_summary('regularization', l2_loss_size * regularization)
...
with tf.Session() as sess:
    ...
    while epoch < n_epoches:
        ...
        acc_valid = sess.run(accuracy_valid)
    ...
    # run on the hidden test set and write to file
    hidden_cls = sess.run(predictions, feed_dict={x: hidden, keep_prob: 1.}) + 1

    filename = os.path.join(ASSIGNMENT_DIR, 'report/results/output.csv')
    write(filename, test_cls, hidden_cls, include_hidden=True)
